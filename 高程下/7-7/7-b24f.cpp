...
int main(int arcg, char *argv[])
{
	...
	/*递归遍历*/
	void dfsFolder(char* folderPath, struct student info[20], struct student info_check, char*filename, char*classnum, char*studentnum, int mode);
	char path[100] = { 0 };
	strcpy(path, ".\\source");
	dfsFolder(path, info, info_check, filename, classnum, studentnum, mode);
	return 0;
}
void dfsFolder(char* folderPath, struct student info[20], struct student info_check, char*filename, char*classnum, char*studentnum, int mode) //mode 0为检查某个源文件 1为检查所有
{
		...
		do {
			...
			if ((FileInfo.attrib & _A_SUBDIR) && (strncmp(classnum, FileInfo.name, 6) == 0))
			{
				if ((strcmp(FileInfo.name, ".") != 0) && (strcmp(FileInfo.name, "..") != 0))
				{
					...
					dfsFolder(newPath, info, info_check, filename, classnum, studentnum, mode);
				}
			}
			else
			{
				...
				if (mode == 1)
				{
					...
					int opflag = 0;
					unsigned int i2;
					for (i2 = 0; i2 < strlen(FileInfo.name); i2++)
					{
						if (FileInfo.name[i2] == '.')
							xsd = i2;
					}
					if (FileInfo.name[xsd + 1] != 'c' && FileInfo.name[xsd + 1] != 'h' || FileInfo.name[xsd + 1] == '\0')
					{
						opflag = 1;
					}
					if (FileInfo.name[xsd + 1] == 'c'&&FileInfo.name[xsd + 2] != 'p'&& FileInfo.name[xsd + 2] != '\0')
					{
						opflag = 1;
					}
					if (FileInfo.name[xsd + 1] == 'c' && FileInfo.name[xsd + 2] == 'p' &&FileInfo.name[xsd + 3] != 'p')
					{
						opflag = 1;
					}
					if (opflag == 0)
